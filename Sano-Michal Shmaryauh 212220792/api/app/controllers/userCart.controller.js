...
exports.delete = (req, res) => {
    UserCart.findByIdAndRemove(req.params.id)
        .then(note => {
            if (!note) {
                return res.status(404).send({
                    message: "Extars not found with id " + req.params.id
                });
            }
            res.send({ message: "Extars deleted successfully!" });
        }).catch(err => {
            ...
            return res.status(500).send({
                message: "Could not delete Extars with id " + req.params.id
            });
        });
};
